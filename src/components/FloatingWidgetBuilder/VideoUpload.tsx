...
export const VideoUpload: React.FC<VideoUploadProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Video Settings - Only show if template supports video */}
      {isVideoSupported && (
        <Card>
          <CardHeader>
      ...
      {/* Show message when video is not supported */}
      {!isVideoSupported && (
        <Alert>
          <Info className="h-4 w-4" />
          <AlertDescription>
            Video functionality is not available for the selected template.
          </AlertDescription>
        </Alert>
      )}

      {/* Video Preview Modal - Only show if template supports video */}
      {isVideoSupported && (
