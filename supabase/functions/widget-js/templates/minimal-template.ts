...
export const minimalTemplate: WidgetTemplate = {
  ...
  html: `
    <div class="widget-container position-{{POSITION}}" id="widget-container">
      <div class="widget-channels-list" id="widget-channels-list">
        {{CHANNELS_HTML}}
      </div>
      <button class="widget-main-button" id="widget-main-button" style="background: {{BUTTON_COLOR}}; --button-size: {{BUTTON_SIZE}}px;">
        {{BUTTON_ICON}}
      </button>
      <div class="widget-tooltip position-{{TOOLTIP_POSITION}} {{TOOLTIP_DISPLAY}}" id="widget-tooltip">
        {{TOOLTIP_TEXT}}
      </div>
      {{VIDEO_CONTENT}}
    </div>
  ...
  js: `
    (function() {
      ...
      function initWidget() {
        ...
        if (!button || !channelsList) {
          console.error('Widget elements not found:', { button: !!button, channelsList: !!channelsList });
          return;
        }

        console.log('Found widget elements:', { button: !!button, channelsList: !!channelsList, tooltip: !!tooltip });
