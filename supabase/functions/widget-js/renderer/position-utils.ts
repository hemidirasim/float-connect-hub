
import type { TemplateConfig } from './types.ts'

export function getPositionStyle(position: string): string {
  switch (position) {
    case 'left':
      return 'left: 20px;'
    case 'center':
      return 'left: 50%; transform: translateX(-50%);'
    case 'right':
    default:
      return 'right: 20px;'
  }
}

export function getTooltipPositionStyle(config: TemplateConfig): string {
  const buttonSize = config.buttonSize || 60
  const tooltipOffset = 12
  const tooltipPosition = config.tooltipPosition || 'top'
  
  // Calculate button offset amounts
  const buttonOffsetX = getButtonOffsetX(config)
  const buttonOffsetY = getButtonOffsetY(config)
  
  let adjustedStyle = ''
  
  switch (tooltipPosition) {
    case 'top':
      // Adjust tooltip position based on button offset
      adjustedStyle = `bottom: ${buttonSize + tooltipOffset + Math.abs(buttonOffsetY)}px; left: calc(50% + ${buttonOffsetX}px); transform: translateX(-50%);`
      break
      
    case 'bottom':
      adjustedStyle = `top: ${buttonSize + tooltipOffset + Math.abs(buttonOffsetY)}px; left: calc(50% + ${buttonOffsetX}px); transform: translateX(-50%);`
      break
      
    case 'left':
      adjustedStyle = `right: ${buttonSize + tooltipOffset + Math.abs(buttonOffsetX)}px; top: calc(50% + ${buttonOffsetY}px); transform: translateY(-50%);`
      break
      
    case 'right':
      adjustedStyle = `left: ${buttonSize + tooltipOffset + Math.abs(buttonOffsetX)}px; top: calc(50% + ${buttonOffsetY}px); transform: translateY(-50%);`
      break
      
    default:
      adjustedStyle = `bottom: ${buttonSize + tooltipOffset + Math.abs(buttonOffsetY)}px; left: calc(50% + ${buttonOffsetX}px); transform: translateX(-50%);`
  }
  
  return adjustedStyle
}

// Helper functions to calculate button offset
function getButtonOffsetX(config: TemplateConfig): number {
  const tooltipPosition = config.tooltipPosition || 'top'
  const position = config.position
  const tooltipText = config.tooltip || ''
  const approximateTooltipWidth = Math.max(120, tooltipText.length * 8)
  
  if (tooltipPosition === 'right' && position === 'right') {
    return -(approximateTooltipWidth / 2 + 20)
  }
  
  if (tooltipPosition === 'left' && position === 'left') {
    return approximateTooltipWidth / 2 + 20
  }
  
  return 0
}

function getButtonOffsetY(config: TemplateConfig): number {
  const tooltipPosition = config.tooltipPosition || 'top'
  
  if (tooltipPosition === 'bottom') {
    return -60
  }
  
  return 0
}

// Function to adjust button position to prevent tooltip from going off-screen
export function getButtonOffsetStyle(config: TemplateConfig): string {
  const offsetX = getButtonOffsetX(config)
  const offsetY = getButtonOffsetY(config)
  
  if (offsetX !== 0 || offsetY !== 0) {
    return `transform: translate(${offsetX}px, ${offsetY}px);`
  }
  
  return ''
}

export function getModalPositionStyle(position: string): string {
  switch (position) {
    case 'left':
      return 'align-items: flex-end; justify-content: flex-start;'
    case 'center':
      return 'align-items: flex-end; justify-content: center;'
    case 'right':
    default:
      return 'align-items: flex-end; justify-content: flex-end;'
  }
}

export function getModalContentPositionStyle(position: string): string {
  switch (position) {
    case 'left':
      return 'bottom: 20px; left: 20px; position: absolute;'
    case 'center':
      return 'bottom: 20px; position: relative; margin: 0 auto;'
    case 'right':
    default:
      return 'bottom: 20px; right: 20px; position: absolute;'
  }
}
