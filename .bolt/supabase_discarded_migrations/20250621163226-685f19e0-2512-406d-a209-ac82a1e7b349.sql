...
VALUES (
  'Minimal Template',
  'Clean and minimalist design with subtle animations',
  '<!-- Minimal HTML Template -->
<div class="hiclient-widget-container minimal-style" style="position: fixed; {{position}}: 20px; bottom: 20px; z-index: 99999;">
  <div class="hiclient-tooltip minimal-tooltip {{tooltip_class}}" style="{{tooltip_style}}">{{tooltip_text}}</div>
  <button class="hiclient-widget-button minimal-button" style="{{button_style}}">
    {{button_icon}}
  </button>
</div>

<div class="hiclient-modal-backdrop minimal-modal">
  <div class="hiclient-modal-content minimal-content">
    <div class="hiclient-modal-header minimal-header">Let''s Connect</div>
    {{video_section}}
    {{channels_section}}
    {{empty_state}}
  </div>
...
.hiclient-widget-container.minimal-style {
  font-family: "SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
}

.hiclient-widget-button.minimal-button {
  width: {{button_size}}px;
  height: {{button_size}}px;
  border-radius: 12px;
  background: {{button_color}};
...
.hiclient-widget-button.minimal-button:hover {
  transform: translateY(-2px);
  box-shadow: 0 8px 25px rgba(0, 0, 0, 0.15);
  border-color: rgba(255, 255, 255, 0.3);
}

.hiclient-tooltip.minimal-tooltip {
...
.hiclient-modal-content.minimal-content {
  border-radius: 16px;
...
.hiclient-modal-header.minimal-header {
  font-weight: 600;
  font-size: 19px;
  color: #1f2937;
  text-align: center;
...
.hiclient-empty-state {
  ...
  '/* Minimal JS Template */
function renderWidget(config) {
  console.log("Rendering minimal widget with config:", config);
}',
  
  true
);

-- Insert dark mode template  
INSERT INTO public.widget_templates (name, description, html_template, css_template, js_template, is_active)
VALUES (
  'Dark Mode Template',
  'Sleek dark theme with vibrant accents',
  '<!-- Dark Mode HTML Template -->
<div class="hiclient-widget-container dark-style" style="position: fixed; {{position}}: 20px; bottom: 20px; z-index: 99999;">
  <div class="hiclient-tooltip dark-tooltip {{tooltip_class}}" style="{{tooltip_style}}">{{tooltip_text}}</div>
  <button class="hiclient-widget-button dark-button" style="{{button_style}}">
    {{button_icon}}
    <div class="glow-effect"></div>
  </button>
</div>

<div class="hiclient-modal-backdrop dark-modal">
  <div class="hiclient-modal-content dark-content">
    <div class="hiclient-modal-header dark-header">Get in Touch</div>
    {{video_section}}
    {{channels_section}}
    {{empty_state}}
  </div>
...
.hiclient-widget-container.dark-style {
  font-family: "Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
}

.hiclient-widget-button.dark-button {
  width: {{button_size}}px;
  height: {{button_size}}px;
  border-radius: 50%;
  background: linear-gradient(135deg, {{button_color}}, #8b5cf6);
  border: none;
  cursor: pointer;
...
.hiclient-widget-button.dark-button:hover {
  transform: scale(1.1);
...
.hiclient-modal-content.dark-content {
  border-radius: 20px;
...
.hiclient-modal-header.dark-header {
  color: #f9fafb;
  font-weight: 600;
  font-size: 20px;
  margin-bottom: 24px;
...
.hiclient-video-player {
  ...
  '/* Dark Mode JS Template */
function renderWidget(config) {
  console.log("Rendering dark mode widget with config:", config);
}',
